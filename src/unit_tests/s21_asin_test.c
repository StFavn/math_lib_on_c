...
static double anomaly_vals[] = {NAN,       // 0
                                INFINITY,  // 1
                                -0.0,      // 2
                                0 / -0.0,  // 3
                                5.5456,   -4.563};

START_TEST(asin_test_anomaly) {
...
Suite *suite_s21_asin() {
  ...
  TCase *normal_case = tcase_create("s21_asin_normal_vals");
  tcase_add_loop_test(normal_case, asin_test_normal, 0,
                      sizeof(normal_vals) / sizeof(normal_vals[0]));
  suite_add_tcase(suite, normal_case);

  TCase *edge_case = tcase_create("s21_asin_edge_vals");
  tcase_add_loop_test(edge_case, asin_test_edge, 0,
                      sizeof(edge_vals) / sizeof(edge_vals[0]));
  suite_add_tcase(suite, edge_case);

  TCase *anomaly_case = tcase_create("s21_asin_anomaly_vals");
  tcase_add_loop_test(anomaly_case, asin_test_anomaly, 0,
