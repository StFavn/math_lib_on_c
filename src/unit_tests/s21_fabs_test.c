...
#define actual(x) s21_fabs(x)
#define expected(x) fabs(x)

static double normal_vals[] = {-1.056, -2e-6, 0, 1.785, 42.21};

START_TEST(fabs_test_normal) {
...
Suite *suite_s21_fabs() {
  Suite *suite = suite_create("suite_s21_fabs");

  TCase *normal_case = tcase_create("s21_fabs_normal_vals");
  tcase_add_loop_test(normal_case, fabs_test_normal, 0,
                      sizeof(normal_vals) / sizeof(normal_vals[0]));
  suite_add_tcase(suite, normal_case);

  TCase *edge_case = tcase_create("s21_fabs_edge_vals");
  tcase_add_loop_test(edge_case, fabs_test_edge, 0,
                      sizeof(edge_vals) / sizeof(edge_vals[0]));
  suite_add_tcase(suite, edge_case);

  TCase *anomaly_case = tcase_create("s21_fabs_anomaly_vals");
  tcase_add_loop_test(anomaly_case, fabs_test_anomaly, 0,
