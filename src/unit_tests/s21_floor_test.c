...
Suite *suite_s21_floor() {
  ...
  TCase *normal_case = tcase_create("s21_floor_normal_vals");
  tcase_add_loop_test(normal_case, floor_test_normal, 0,
                      sizeof(normal_vals) / sizeof(normal_vals[0]));
  suite_add_tcase(suite, normal_case);

  TCase *edge_case = tcase_create("s21_floor_edge_vals");
  tcase_add_loop_test(edge_case, floor_test_edge, 0,
                      sizeof(edge_vals) / sizeof(edge_vals[0]));
  suite_add_tcase(suite, edge_case);

  TCase *anomaly_case = tcase_create("s21_floor_anomaly_vals");
  tcase_add_loop_test(anomaly_case, floor_test_anomaly, 0,
